...
### The phone, with nothing to set up: Share over the internet

The zero-step way to reach the laptop at home from your phone anywhere: **Settings → Devices and remote access → Share over the internet**. The Control Plane downloads `cloudflared` on first use (a pinned release, SHA-256 checked, kept in `~/.sessionboxer/bin`; an installed `cloudflared` on the PATH is used instead) and runs a Cloudflare **quick tunnel** to itself: an outbound connection that gives it a random public address such as `https://maria-florida-citation.trycloudflare.com`, with a real certificate, no Cloudflare account, no port forwarding, no DNS. The switch shows *starting…*, then the address; from then on **Pair another device** makes its QR and link with that address, so the whole phone side is: scan, tap, logged in. The tunnel stays on across restarts of the Control Plane until you switch it off, and `cloudflared` stops with the Control Plane; if it dies it is restarted with back-off and the switch shows the last error.

What that gives and costs: the address is public, so the access token is the only wall — the Sessionboxer login is required exactly as before, nothing is opened up (anonymous requests through the tunnel get a 401, and the tunnel's own forwarded headers are only believed on requests that arrive from `cloudflared` on this machine with the tunnel's hostname). All traffic passes through Cloudflare's edge, TLS-terminated there and re-encrypted to your machine, and **the address changes every time the tunnel starts** (a phone already paired stays logged in — its cookie is for the device, not the address — but the bookmark goes stale, so pair again or read the new address in Settings). Quick tunnels are Cloudflare's try-out tier: no uptime promise, rate limits, and they may be discontinued or changed by Cloudflare; for something permanent use a named Cloudflare Tunnel on your own domain (below) or a private network. `SESSIONBOXER_PUBLIC_URL` is not touched by the tunnel — configured links and the GitHub OAuth callback keep using it. `SESSIONBOXER_CLOUDFLARED_VERSION` picks another release (its checksums are then read from the release notes).
...
- **A permanent URL that works from any browser: a named Cloudflare Tunnel.** The grown-up version of *Share over the internet*: `cloudflared tunnel` on the server keeps an outbound connection to Cloudflare, which terminates TLS on a hostname of a domain you have there and forwards to `http://127.0.0.1:4000`; put **Cloudflare Access** in front for a second login (email code, Google, GitHub). Set `SESSIONBOXER_PUBLIC_URL=https://box.yourdomain.tld` and `SESSIONBOXER_TRUST_PROXY=1`. Cloudflare drops idle WebSockets after 100 s; the Control Plane pings every 25 s, so terminals and the desktop survive.
...
| Downloaded `cloudflared` (Share over the internet) | `~/.sessionboxer/bin/cloudflared` |
