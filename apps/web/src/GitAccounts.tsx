...
import { CONNECTOR_KINDS, CONNECTORS, type ConnectorKind, type PublicMcpServerDef, type PublicSettings } from "@sessionboxer/protocol";
import { api } from "./api";
...
export function GitAccounts({
  ...
  return (
    <fieldset className="choice" id="settings-git">
      ...
      <p className="muted">
        Connect the account Sessions use to clone, push and open pull requests. GitHub offers three logins: GitHub CLI (everything your
        account sees), the Sessionboxer OAuth App (you grant organizations one by one on GitHub's page) and a personal access token (the
        one that can be limited to a single organization); the dialog explains each. A GitHub account also adds GitHub's MCP server to
        the list below.
      </p>
      {accounts.length === 0 && <p className="muted empty-inline">No Git account connected yet.</p>}
      {accounts.length > 0 && (
          ...
          {accounts.map((s) => {
            ...
            return (
              <li key={s.id} className="mcp-card connector">
                ...
                {c.account ? (
                  <span className="mcp-summary connector-status" title={`Connected ${new Date(c.connectedAt ?? 0).toLocaleString()}`}>
                    Connected as <strong>@{c.account}</strong>
                    {c.host && <span className="muted"> on {c.host}</span>}
                    {c.expiresAt && Date.parse(c.expiresAt) < Date.now() && <span className="connector-expired"> (token expired)</span>}
                  </span>
                ) : (
                  <span className="muted mcp-summary">Not connected</span>
                )}
                <span className="spacer" />
                <button type="button" className="small" onClick={() => setConnecting({ kind: c.kind, server: s })}>
      ...
      <div className="mcp-actions">
        {CONNECTOR_KINDS.map((kind) => (
          <button key={kind} type="button" className="connector-button" onClick={() => setConnecting({ kind, server: null })}>
