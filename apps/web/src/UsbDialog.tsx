...
export function UsbDialog({ session, sessions, onClose }: { session: Session; sessions: Session[]; onClose: () => void }) {
  ...
  return (
    <Modal className="usb-dialog" title={`USB devices for "${session.title}"`} dismissible={busy === null} onClose={onClose}>
      <p className="muted">
      ...
      <ul className="list-box usb-list" aria-busy={busy !== null || host === null}>
