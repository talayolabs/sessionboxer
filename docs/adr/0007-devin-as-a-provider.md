...
Authenticated run through the Control Plane (Devin Session, `WINDSURF_API_KEY` from Settings, `swe-1-6-fast`): `session/new` → `bypass` mode → a real turn in which Devin called `desktop` `screenshot`, `left_click`, `type`, `key`, `wait`, `screenshot` to open Wikipedia in Firefox and described the page; screenshots arrived as ACP image content in `tool_call_update`, the same shape Claude produces; Devin also wrote `/workspace/heading.txt` via its `write` tool (shown as a diff tool call). Stop → Resume reloaded the ACP session (`session/load`, mode already `bypass`, transcript intact). Two caveats observed: Devin's first `session/new` right after container boot failed once with `Failed to fetch team settings: fetch timed out after 10000ms` and succeeded on the next prompt (the Daemon respawns the Agent), and Devin's stderr is verbose `INFO` tracing that ends up in the Daemon log.
