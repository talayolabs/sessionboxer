...
- The supported set is a table in the protocol package (`mediaKind` / `contentTypeFor`): video (mp4, m4v, webm, mov), audio (mp3, wav, ogg, m4a), image (png, jpg, jpeg, gif, webp, svg), pdf. Mermaid is not a file the browser renders natively and was left for later; ADR-0018 adds `markdown` (md, markdown) and `mermaid` (mmd, mermaid) kinds rendered client-side.
...
- Not done: thumbnails/posters generated in the box, transcoding of formats browsers cannot play (a `.mov` with ProRes will show a broken player; ffmpeg is there for the agent to convert), a size cap on embeds (a 2 GB video is streamed on demand, so it is only a bandwidth concern).
