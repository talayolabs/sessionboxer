...
- `pad` adds a band under the desktop (a quarter of the display height: 192 px for 1024×768) so no pixel of the recorded screen is covered.
- The captions are an ASS script sized to the padded frame with one style: DejaVu Sans at a seventh of the band height (27 px), light on the band's colour, **top-centre aligned with the vertical margin at the band's upper edge**. Anchoring at the top means a caption that wraps grows *down into the band*, which is what the prototype (bottom-anchored, `MarginV` from the frame's bottom) got wrong: its second line climbed into the desktop. The band holds four to five lines of that size, so a two-line caption sits comfortably; text is capped at 300 characters and braces/backslashes (ASS override syntax) are stripped.
...
- **Overlay captions on the desktop** (rejected): covers the very UI the video is about; the band costs 25 % more pixels and nothing else.
- **Bottom-anchored text in the band** (rejected, prototype): multi-line captions grow upwards into the desktop.
...
- Videos with captions are 1024×960 instead of 1024×768; the band is part of the file the user downloads, so the captions travel with it.
