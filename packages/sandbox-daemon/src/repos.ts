...
export class Repos {
  ...
  async set(params: DaemonReposSetParams): Promise<void> {
    const manifest: ReposManifest = {
      ...
      repos: params.repos.map((r) => ({ name: r.name, path: r.name === "." ? this.workspace : join(this.workspace, r.name), source: r.source })),
    };
  ...
  briefing(): string {
    ...
    const rows = repos.map((r) => `- \`${r.path}\` — ${r.source.type === "git" ? "clone of" : "copy of"} ${repoOriginLabel(r.source)}`);
    return [
